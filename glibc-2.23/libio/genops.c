...
int _IO_sputbackc(_IO_FILE *fp, int c)
{
  ...
  if (fp->_IO_read_ptr > fp->_IO_read_base && (unsigned char)fp->_IO_read_ptr[-1] == (unsigned char)c)
  {
    fp->_IO_read_ptr--;
    ...
    int _IO_flush_all_lockp(int do_lock)
{
  ...
  while (fp != NULL)
  {
    ...
    if (((fp->_mode <= 0 && fp->_IO_write_ptr > fp->_IO_write_base)
        ...
        _IO_OVERFLOW(fp, EOF) == EOF) // 指向伪造的vtable
      result = EOF;
...
{
  if (fp->_IO_read_ptr > fp->_IO_read_base && !_IO_in_backup(fp) && (unsigned char)fp->_IO_read_ptr[-1] == c)
    --fp->_IO_read_ptr;
  else
  {
...
libc_hidden_def(_IO_iter_next)
    ...
    _IO_FILE *_IO_iter_file(_IO_ITER iter)
{
  return iter;
}
