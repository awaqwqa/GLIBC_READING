...
{
    // TIPS[unlink]:检查下一个chunkprev_size和size是否相同
    if (chunksize(p) != prev_size(next_chunk(p)))
        malloc_printerr("corrupted size vs. prev_size");

    mchunkptr fd = p->fd;
    mchunkptr bk = p->bk;
    // 检查fd指向的chunk的bk是否指向自己 bk指向的chunk的fd是否指向自己
    if (__builtin_expect(fd->bk != p || bk->fd != p, 0))
        malloc_printerr("corrupted double-linked list");
    // 进行脱链
    fd->bk = bk;
    bk->fd = fd;
    // 如果是largebin范围 且fd_nextsize不为0
    if (!in_smallbin_range(chunksize_nomask(p)) && p->fd_nextsize != NULL)
    {
        // 注意largebin的不同大小区间的头chunk之间是通过fd_nextsize和bk_nextsize形成的双向链表进行链接的
        // 检查fd_nextsize的bk_nextsize是否为自己 bk_nextsize的fd_nextsize是否为自己
        if (p->fd_nextsize->bk_nextsize != p || p->bk_nextsize->fd_nextsize != p)
            malloc_printerr("corrupted double-linked list (not small)");
        // 如果fd的fd_nextsize为0
        if (fd->fd_nextsize == NULL)
        {
            // 如果只有一个大小区间的largebinchunk链
            if (p->fd_nextsize == p)
                // 将这个大小区间中第二个chunk的fd_nextsize和bk_nextsize都等于fd 完成脱链
...
// 检查chunk是否为main_arena区 是否大小大于最小chunk p位是否为1
...
sysmalloc(INTERNAL_SIZE_T nb, mstate av)
{
     ...
     */
    //   也就是说MINSIZE<old_top_size<nb+MINSIZE
    //   old_top_size的prev_size位是
    //   (old_top_size+old_top)&0xfff=0x000
    //   nb<0x20000 大于这个区间就去mmap了
...
#define REMOVE_FB(fb, victim, pp)                                          \
    ...
    {
        // 获取fastbin链的索引
        // fastbin链和tcache链是单独分配的 而不是和其他链一起在bin链当中
        idx = fastbin_index(nb);
        // 因为fastbin链中是按照大小来进行分组的 所以我们根据大小获取index 然后从fastbin链中找到对应坑位 这个坑位就是一个fastbin链
        // 获取头链
        ...
        if (victim != NULL)
        {
            ...
            {
                // TIPS[fastbin]: 根据头chunk的size大小获取index 检查index和所对应的fastbin链所属是否相同 双重保证
                size_t victim_idx = fastbin_index(chunksize(victim));
                if (__builtin_expect(victim_idx != idx, 0))
                    malloc_printerr("malloc(): memory corruption (fast)");
                // TIPS[fastbin]:检测prev_inuse位置是否为1 也就是查看chunk是否处于使用状态 这个需要特殊注意
                check_remalloced_chunk(av, victim, nb);
                // 当使用tcache情况下
#if USE_TCACHE
                /* While we're here, if we see other chunks of the same size,
               stash them in the tcache.  */
                // TIPS[fastbin]:当tcache存在 如果tcache链中有空闲 就会执行fastbin的入链操作
     ...
     */
    // 如果是再small chunk smallbin是优先从最后一个chunk进行拖链的
    if (in_smallbin_range(nb))
    {
        ...
        {
            // 多个chunk的情况下 取根据最后一个chunk
            bck = victim->bk;
            // TIPS[smallbin]:然后检查最后一个chunk前方的chunk的fd是否指向最后一个chunk
            ...
            // 让尾部指向倒数第二个chunk
            bin->bk = bck;
            // 让倒数第二个的chunk指向bin 完成拖链
            bck->fd = bin;
            // 如果当前arena不是主分配区 则标记为这个chunk为非主分配区进行分配的
            if (av != &main_arena)
                set_non_main_arena(victim);
            // TIPS[smallbin]:检测victim是否正常 检测是否p位置为1 检查内存是否对齐
            ...
            if (tcache && tc_idx < mp_.tcache_bins)
            {
                ...
                // 和fastbin一模一样 如果samllbin有空闲chunk 然后tcache没有满则直接入tcache
    ...
    {
        // 如果属于largebin chunk范围也是先拿到
        idx = largebin_index(nb);
        // 如果存在fastbin 的chunks 则进行fastbin的合并机制
     ...
     */
// 如果使用了tcache机制则 当tcache对应大小区间的链还空时直接入tcache中
#if USE_TCACHE
    ...
    {
        int iters = 0;
        // victim 是正在遍历的unsrotedbin chunk 从最后一个开始遍历 只要不等于unsroedbin头则继续遍历
        while ((victim = unsorted_chunks(av)->bk) != unsorted_chunks(av))
        {
             ...
             */
            // smallbin 情况 而且smallchunk中仅有一个chunk size大于最小chunk
            if (in_smallbin_range(nb) &&
                ...
                (unsigned long)(size) > (unsigned long)(nb + MINSIZE))
            {
                ...
                // unsortedbin的fd和bk都指向remainder
                unsorted_chunks(av)->bk = unsorted_chunks(av)->fd = remainder;
                // 剩余chunk指针
                av->last_remainder = remainder;
                // 剩余的chunk入链unsortedbin中
            ...
            // TIPS[unsortedbin]:当前一个chunk的fd不等于当前chunk的时候
            /* remove from unsorted list */
            if (__glibc_unlikely(bck->fd != victim))
                malloc_printerr("malloc(): corrupted unsorted chunks 3");
            // 直接从unsortedbin中删除当前chunk
            // 先绑定unsrotedbin的bk和前一个chunk的fd
            // 重点位置 攻击地点
            unsorted_chunks(av)->bk = bck;
            bck->fd = unsorted_chunks(av);

            /* Take now instead of binning if exact fit */
            // 当遍历的chunk size刚好符合的时候
            if (size == nb)
            {
                // 将物理意义上的下一个chunk的prev_inuse位设置为1
                // 也就是标记当前chunk为free状态
            ...
            We may return one of these chunks later.  */
                //  当有对应的tcache链的且该链未满的情况下直接将当前chunk放入tcache中 而不进行返回 开始遍历下一次循环 所以这里当大小相等的时候 如果原本tcache就有则会在函数早期直接获取返回了
                // 如果tcache中无 但是unsortedbin中有一样的会直接进入tcache中 然后再从tcache中返回
            ...
            else
            {
                ...
                if (fwd != bck)
                {
                    ...
                    // 对size进行合法检查和检查是否为main_arena分配
                    assert(chunk_main_arena(bck->bk));
                    // 如果当前遍历的chunk小于 largebin链中最小的size chunk 则直接加入largebin链表尾巴中
                    if ((unsigned long)(size) < (unsigned long)chunksize_nomask(bck->bk))
                    {
                        // TIPS[largebin]:largebin attack的位置
                        // 此时fwd直接等于bck 然后back->bk也就是最小的size chunk
                        fwd = bck;
                        bck = bck->bk;
                        // fwd->fd也就是largebin size chunk 中最小的chunk
                        victim->fd_nextsize = fwd->fd;
                        // 最小chunk的bk_nextsize赋值给当前遍历chunk的bk_nextsize
                        victim->bk_nextsize = fwd->fd->bk_nextsize;
                        // 这里victim->bk_nextsize->fd_nextsize等效为 最小chunk的bk_nextsize位置的fd_nextsize偏移处等于victim 直接泄露了地址 而且没有过多的安全检测
                        fwd->fd->bk_nextsize = victim->bk_nextsize->fd_nextsize = victim;
                    }
                    else
                    {
                        // 如果要利用这里 会限制巨多 各种安全检测
                        // 否则开始遍历
                        assert(chunk_main_arena(fwd));
                        // 从最大的chunk开始遍历 直到遇到一个最小的比当前遍历chunk小于等于的largebin size chunk后
                        // 所以当我们size chunk链中完全最大的chunk都没有遍历的chunk大 那么就不会调用fwd->fd_nextsize
                        while ((unsigned long)size < chunksize_nomask(fwd))
                        {
                            fwd = fwd->fd_nextsize;
                            assert(chunk_main_arena(fwd));
                        }
                        // 如果说两个的size刚刚好 则直接加在这个size chunk后方
                        if ((unsigned long)size == (unsigned long)chunksize_nomask(fwd))
                            /* Always insert in the second position.  */
                            fwd = fwd->fd;
                        else
                        {
                            // 当前遍历的chunk是大于这个largebin size chunk的时候 那么就当作size chunk 进入largebin的链头中(size chunk指的是 largebin中通过fd_nextsize bk_nextsize维护的双向链表中的chunk 这些chunk都通过fd和bk维护一个chunk大小相同的双向链表chunk)
                            // 直接把当前遍历chunk的fd_nextsize = fwd bk_nextsize等于fwd->bk_nextsize
                            victim->fd_nextsize = fwd;
                            victim->bk_nextsize = fwd->bk_nextsize;
                            // size chunk的bk_nextsize的fd_nextsize是否等于自己本身 也就是通过前一个chunk来进行校验自己的正确性
                            // 所以一般情况下我们需要这个恰好小一点的siezchunk是合法入的largebin链中 然后控制这个size chunk恰好比我们遍历的chunk1大一点
                            // 这里检验的是fwd->bk_nextsize->fd_nextsize所以我们要控制bk_nextsize指向一个可控区域再让对应偏移位置地址为fwd 这里多半要求我们能够泄露libc地址或者能够在bss段操作
                            if (__glibc_unlikely(fwd->bk_nextsize->fd_nextsize != fwd))
                                malloc_printerr("malloc(): largebin double linked list corrupted (nextsize)");
                            // fwd->bk_nextsize等于victim
                            fwd->bk_nextsize = victim;
                            // victim的bk_nextsize也就是fwd->bk_nextsize 所以这里等效为fwd->bk_nextsize->fd_nextsize等于victim 这样就完成了向我们可控区域写入了堆地址
...
#define MAX_ITERS 10000
         ...
         */
        // 如果是largebin 取出情况 这里的victim是
        if (!in_smallbin_range(nb))
        {
            bin = bin_at(av, idx);
            // 如果largebin对应大小区间的最大的size chunk大于所需要chunk大小
            /* skip scan if empty or largest chunk is too small */
            if ((victim = first(bin)) != bin && (unsigned long)chunksize_nomask(victim) >= (unsigned long)(nb))
            {
                // 从最小的chunk开始遍历
                victim = victim->bk_nextsize;
                // 遍历到刚好大于所需要chunk的size chunk
                while (((unsigned long)(size = chunksize(victim)) <
                        (unsigned long)(nb)))
                    // size刚好为这个恰好大于的size chunk的size
                    victim = victim->bk_nextsize;

                /* Avoid removing the first entry for a size so that the skip
                   list does not have to be rerouted.  */
                // 如果这个刚好大于的chunk不是最小chunk 而且这个size chunk所对应的链有第二个chunk
        ...
        map = av->binmap[block];
        bit = idx2bit(idx);
        // 这部分就是从bin中寻找一个刚好大于的chunk塞进去了 并且会进行分割
        for (;;)
        {
    ...
    use_top:
        ...
        // 如果 fastbin中还有chunk 则合并fastbin然后从small/largebin中取
...
_int_free(mstate av, mchunkptr p, int have_lock)
{
    ...
    */
    ...
    {
        // 根据偏移得到 chunk 也就是p+size 然后获取size并且去除flags
    ...
    */
    ...
    else if (!chunk_is_mmapped(p))
    {
        ...
        // 如果说nextchunk的大小大于了topchunk+其大小的大小，说明越界了
...
static void malloc_consolidate(mstate av)
{
    ...
    int nextinuse;
    // 原子操作将是否存在fastbinchunk标志位改为false
    atomic_store_relaxed(&av->have_fastchunks, false);
    // 获取unsoretedbin的链头
    ...
    */
    ...
    // 获取最小大小的fastbin头链
    fb = &fastbin(av, 0);
    // 这里是循环 从最小fastbin 链开始遍历一直到最大的fastbinchunk链 fb就是不同大小区间的头链chunk
    do
    {
        p = atomic_exchange_acq(fb, NULL);
        if (p != 0)
        {
            // 这里的循环代表这个大小区间 fastbin中的每个chunk 所以p就是bin链中的遍历chunk
            do
            {
                {
                    // 检查内存是否对齐
                ...
                // 获取第二个chunk
                nextchunk = chunk_at_offset(p, size);
                // 获取物理顺序上下一个chunk的size
                nextsize = chunksize(nextchunk);
                // 当前遍历chunk的prev_inuse为0会触发合并机制
                if (!prev_inuse(p))
                {
                    prevsize = prev_size(p);
                    size += prevsize;
                    // 获取要合并的chunk
                    p = chunk_at_offset(p, -((long)prevsize));
                    // TIPS[fastbin合并]:检查要合并的chunk的size和prev_size是否相同
                    if (__glibc_unlikely(chunksize(p) != prevsize))
                        malloc_printerr("corrupted size vs. prev_size in fastbins");
                    // 脱链
                    unlink_chunk(av, p);
                }
                // 如果下一个chunk不是topchunk
                if (nextchunk != av->top)
                {
                    // 查看下一个chunk的下一个chunk的prev_inuse标志 也就是查看下一个chunk是否为free状态
                    nextinuse = inuse_bit_at_offset(nextchunk, nextsize);
                    if (!nextinuse)
                    {
                        // 直接通过unlink进行合并
                        ...
                        // 否则直接设置下一个chunk的prev_inuse位为0
                        clear_inuse_bit_at_offset(nextchunk, 0);
                    // 获取第一个unsortedbin chunk的地址
                    first_unsorted = unsorted_bin->fd;
                    // 让unsoretdbin首个chunk变成当前chunk
                    unsorted_bin->fd = p;
                    // 让原第一个chunk的bk指向当前遍历chunk
                    first_unsorted->bk = p;
                    // 处于largebin中则直接清空fd_nextsize和bk_nextsize
                    if (!in_smallbin_range(size))
                    {
                        p->fd_nextsize = NULL;
                        p->bk_nextsize = NULL;
                    ...
                    // 让fd指向原本的第一个chunk
                    p->fd = first_unsorted;
                    // TIPS[fastbin合并]:设置下一个chunk的prev_size位置
                    set_foot(p, size);
                }

                else
                {
                    // 如果为topchunk 则直接合并进入topchunk中
                    size += nextsize;
                    set_head(p, size | PREV_INUSE);
                    av->top = p;
                }
