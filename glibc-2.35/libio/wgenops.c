...
{
  if (fp->_wide_data->_IO_read_ptr > fp->_wide_data->_IO_read_base && !_IO_in_backup(fp) && (wint_t)fp->_IO_read_ptr[-1] == c)
    --fp->_IO_read_ptr;
  else
  {
    ...
    if (!_IO_in_backup(fp))
    {
      ...
      if (fp->_wide_data->_IO_read_ptr > fp->_wide_data->_IO_read_base && _IO_have_wbackup(fp))
      {
      ...
      else if (!_IO_have_wbackup(fp))
      {
        ...
        fp->_wide_data->_IO_save_end = (fp->_wide_data->_IO_save_base + backup_size);
    ...
    else if (fp->_wide_data->_IO_read_ptr <= fp->_wide_data->_IO_read_base)
    {
      ...
      size_t new_size;
      size_t old_size = (fp->_wide_data->_IO_read_end - fp->_wide_data->_IO_read_base);
      wchar_t *new_buf;
      new_size = 2 * old_size;
    ...
    static int save_for_wbackup(FILE *fp, wchar_t *end_p)
{
  ...
  size_t current_Bsize = (fp->_wide_data->_IO_save_end - fp->_wide_data->_IO_save_base);
...
_IO_sputbackwc(FILE *fp, wint_t c)
{
  ...
  if (fp->_wide_data->_IO_read_ptr > fp->_wide_data->_IO_read_base && (wchar_t)fp->_wide_data->_IO_read_ptr[-1] == (wchar_t)c)
  {
...
void _IO_init_wmarker(struct _IO_marker *marker, FILE *fp)
{
  ...
  if (_IO_in_backup(fp))
    ...
    marker->_pos = (fp->_wide_data->_IO_read_ptr - fp->_wide_data->_IO_read_base);
...
int _IO_wmarker_delta(struct _IO_marker *mark)
{
  ...
  if (_IO_in_backup(mark->_sbuf))
    cur_pos = (mark->_sbuf->_wide_data->_IO_read_ptr - mark->_sbuf->_wide_data->_IO_read_end);
  else
    cur_pos = (mark->_sbuf->_wide_data->_IO_read_ptr - mark->_sbuf->_wide_data->_IO_read_base);
...
int _IO_seekwmark(FILE *fp, struct _IO_marker *mark, int delta)
{
  ...
  if (mark->_pos >= 0)
  {
    if (_IO_in_backup(fp))
      _IO_switch_to_main_wget_area(fp);
    fp->_wide_data->_IO_read_ptr = (fp->_wide_data->_IO_read_base + mark->_pos);
  }
  else
  {
    if (!_IO_in_backup(fp))
